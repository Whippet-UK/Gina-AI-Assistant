...
const PLAN_RULES: Array<{intent:string; mode:CapabilityMode; match:RegExp; capabilities:string[]; actions:string[]}> = [
  ...
  {intent:'web-app-build',mode:'act',match:/^(?!.*\b(?:how do i|how can i|what is|explain|show me how)\b).*\b(?:build|scaffold|develop|create|make|generate)\b.*\b(?:web app|website|react app|react dashboard|frontend|full[- ]stack app|vite app|dashboard|web application)\b/i,capabilities:['filesystem.read','filesystem.write','filesystem.mkdir','execution.command','project.validate','project.integrity'],actions:['list_directory','read_file','create_directory','write_file','execute_command','validate_project','project_integrity_check','git_diff']},
...
export function planCapabilityIntent(text:string, registry:{capabilities:CapabilityRecord[]}): CapabilityPlan {
  ...
  const actionLike=/\b(?:edit|modify|change|update|patch|repair|fix|implement|refactor|rewrite|replace|remove|delete|create|make|build|scaffold|develop|write|save|rename|move|copy|add|append|prepend)\b/i.test(q);
