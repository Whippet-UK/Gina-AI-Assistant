...
const knownModels: ModelSeed[] = [
  ...
  { id:'qwen3.5-mmproj', fileName:'mmproj-F16.gguf', category:'projector', relative:'..\\models\\llm\\mmproj-F16.gguf', purpose:'Qwen3.5-9B matched F16 vision projector', enabled:true },
];
...
export function buildCapabilities(args:{hardware:any; comfy:any; models:LocalModel[]; workflows:any[]; customNodes?:any[]; nodeClasses?:string[]}):LocalCapabilities{
  ...
  const qwen35Mmproj=has('qwen3.5-mmproj')||hasLike(/mmproj-f16.*\.gguf$/i)||hasLike(/qwen3\.5.*mmproj.*\.gguf$/i);
  ...
  return {
    ...
    generators:[
      ...
      {id:'qwen3.5-local',label:'Qwen3.5 9B General + Vision Engine',type:'llm',status:qwen35&&qwen35Mmproj?'validated':qwen35?'installed':'unavailable',workflowIds:[],modelIds:args.models.filter(m=>m.exists&&(/qwen3\.5/i.test(m.fileName)||/mmproj-f16/i.test(m.fileName)||/qwen3\.5.*mmproj/i.test(m.fileName))).map(m=>m.id),notes:['Q4_K_M model; vision is validated only when a Qwen3.5-9B-matched projector is present. The known-incompatible generic mmproj-BF16.gguf is ignored.']},
