...
export interface LocalCapabilities {
  ...
  runtime: { qwenVisionReady?: boolean; qwenCoderReady?: boolean; qwen35Ready?: boolean; juggernautReady?: boolean; comfyConnected: boolean; gpuAvailable: boolean; ggufReady:boolean; gifStudioReady:boolean; rifeReady:boolean; wanReady:boolean; nodeGraphSyncReady:boolean };
}
...
const knownModels: ModelSeed[] = [
  ...
  { id:'wan-video-1.3b', fileName:'wan2.1_t2v_1.3B_bf16.safetensors', category:'unet', relative:'models/checkpoints/wan2.1_t2v_1.3B_bf16.safetensors', purpose:'Wan 2.1 1.3B BF16 native video generation', enabled:true, aliases:['wan2.1_t2v_1.3B_bf16.safetensors', 'wan2.1-1.3b.safetensors'] },
...
function classifyModel(fileName:string, rel:string): {category:LocalModel['category']; purpose:string; id:string; note?:string}|null {
  ...
  if (n.includes('vace')) return {category:'checkpoint',purpose:'Wan 2.1 VACE 1.3B FP16 video model with character anchor control',id:`wan-vace-${fileName.toLowerCase().replace(/[^a-z0-9]+/g,'-')}`};
...
export function buildCapabilities(args:{hardware:any; comfy:any; models:LocalModel[]; workflows:any[]; customNodes?:any[]; nodeClasses?:string[]}):LocalCapabilities{
  ...
  const wan=hasLike(/wan2\.1.*1\.3b|wan.*vace|wan_2\.1_vae|umt5_xxl_fp8_e4m3fn_scaled/i);
  ...
  return {
    ...
    runtime:{
      ...
      qwen35Ready:qwen35&&qwen35Mmproj,
    ...
    generators:[
      ...
      {id:'wan-video',label:'Wan 2.1 1.3B Native Video',type:'video',status:wan&&wanW.length?'validated':wan?'installed':'unavailable',workflowIds:wanW,modelIds:args.models.filter(m=>m.exists&&/wan2\.1|wan_2\.1|vace|umt5_xxl/i.test(m.fileName)).map(m=>m.id),notes:['Native ComfyUI Wan 2.1 workflow with local UMT5, Wan VAE, and VACE reference control.']},
