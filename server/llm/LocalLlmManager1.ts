...
export class LocalLlmManager {
  ...
  private async resolveMmprojPath():Promise<string|null>{if(this.config.mmprojPath){this.resolvedMmprojPath=await fs.stat(this.config.mmprojPath).then(s=>s.isFile()?this.config.mmprojPath!:null).catch(()=>null);return this.resolvedMmprojPath;}const root=path.dirname(this.config.modelPath);try{const files=await fs.readdir(root);const candidates=files.filter(name=>/mmproj.*\.gguf$/i.test(name)); const ranked=candidates.sort((a,b)=>{const score=(name:string)=>this.engine==='qwen' ? ((/qwen/i.test(name)?20:0)+(/f16/i.test(name)?10:0)) : ((/gemma/i.test(name)?20:0)+(/q8_0/i.test(name)?10:0)); return score(b)-score(a);}); const match=ranked[0]; this.resolvedMmprojPath=match?path.join(root,match):null;return this.resolvedMmprojPath;}catch{return null;}}
