...
const ENGINE_META: Record<LocalLlmEngine, { label:string; model:string; detail:string; vision:boolean }> = {
  ...
  gemma: { label:'Gemma 3 12B', model:'gemma-3-12b-it-Q4_K_M.gguf', detail:'Secondary · Text instruction model', vision:false }
};
...
export const AiStudioSuite: React.FC<Props> = ({ onAddLog, view }) => {
  ...
  return <section className="bg-slate-900/50 border border-slate-800 rounded-lg p-4 mb-5 shadow-sm">
    ...
    {tab==='creator' && <div className="mb-3 rounded-lg border border-slate-800 bg-slate-950/80 p-3">
      <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><Cpu className="w-3.5 h-3.5 text-emerald-400"/><div><div className="text-[10px] font-bold tracking-widest text-slate-200 uppercase">LOCAL AI ENGINE</div><div className="text-[9px] text-slate-500 font-mono">http://127.0.0.1:{llm.port || 8080} · Qwen default</div></div></div><div className={`text-[9px] font-bold px-2 py-1 rounded border ${llm.ready?'text-emerald-400 border-emerald-500/30 bg-emerald-500/5':'text-amber-400 border-amber-500/30 bg-amber-500/5'}`}>{llm.ready?'READY':'STOPPED'}</div></div>
