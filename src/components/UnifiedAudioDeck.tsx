...
export const UnifiedAudioDeck: React.FC<{ onAddLog?: (level:'INFO'|'WARN'|'SEC'|'RULE', message:string) => void }> = ({ onAddLog }) => {
  const [engine, setEngine] = useState<'bark'|'xtts_v2'>('bark');
  ...
  const [generationProgress, setGenerationProgress] = useState<{stage:string; percent:number; message:string; engine?:string; row?:number; totalRows?:number} | null>(null);
  ...
  const drop = useDropzone({ onDrop: files => { if (files[0]) void uploadClone(files[0]); }, accept:{'audio/wav':['.wav'],'audio/mpeg':['.mp3']}, multiple:false, disabled:activeVoiceEngine !== 'xtts_v2' });
  ...
  const previewVoice = async (voice:VoiceRecord) => {
    setError(null);
    try {
      const previewEngine = voice.engine_compatibility.includes('bark') ? 'bark' : 'xtts_v2';
      if (previewEngine === 'xtts_v2' && !voice.xtts_embedding_path && !voice.speaker_name) throw new Error('This XTTS voice has no speaker ID or reference recording.');
      const r = await fetch('/api/audio/preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({voice_id:voice.voice_id,engine:previewEngine,voice_preset:voice.bark_prompt_path || undefined,speaker:voice.speaker_name,speaker_wav:voice.xtts_embedding_path || undefined})});
  ...
  const generate = async () => {
    ...
    try {
      ...
      for (let i=0; i<Math.max(1,Math.min(4,batchSize)); i++) {
        ...
        if (useAudioStream) {
          ...
          setGenerationProgress({stage:'DOWNLOADING', percent:100, message:'Generation complete; receiving audio…'});
          const blob = await r.blob();
          const url = URL.createObjectURL(blob);
          results.push({ok:true,url,path:'streamed',bytes:blob.size,seed:Number(payload.seed)});
        } else {
          if (!r.ok || !r.body) { const failure = await r.json().catch(() => ({})); throw new Error(failure.error || `Generation failed (HTTP ${r.status})`); }
          ...
          while (true) {
            ...
            for (const line of lines) {
              ...
              else if (event.type === 'error') throw new Error(event.error || 'Generation failed.');
            }
            if (chunk.done) break;
          }
          if (buffer.trim()) {
          ...
          if (!finalResult?.ok) throw new Error(finalResult?.error || 'Generation completed without an audio result.');
    ...
    } catch (e:any) { setError(e?.message || 'Unified audio generation failed.'); setGenerationProgress(prev => prev ? {...prev, stage:'ERROR', message:e?.message || 'Generation failed.'} : {stage:'ERROR', percent:0, message:e?.message || 'Generation failed.'}); }
