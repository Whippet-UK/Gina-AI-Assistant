...
<<<<<<< HEAD
    ...
    { id: 'RESTORE_V1.17.77_AUDIOCRAFT_EXPLICIT_BACKEND', label: 'AudioCraft Explicit Backend & Sequential Generation', description: 'Explicit Transformers/AudioCraft backend reporting, managed-only model resolution, Hub duplicate exclusion, offline Generate, and single-lane GPU execution', timestamp: new Date().toISOString().slice(0,16).replace('T',' '), status: 'LOCKED' },
    { id: 'RESTORE_V1.17.78_MUSICGEN_HF_SNAPSHOT', label: 'MusicGen Transformers Snapshot Routing', description: 'Uses the already-downloaded MusicGen Medium Transformers snapshot containing model.safetensors; Generate is local-only, never downloads, and audio remains single-lane', timestamp: new Date().toISOString().slice(0,16).replace('T',' '), status: 'LOCKED' },
    { id: 'RESTORE_V1.17.79_MUSICGEN_STATUS_RESOLUTION', label: 'MusicGen Snapshot Status Resolution', description: 'Status/backend telemetry resolves the same local Hugging Face snapshot used by generation, reports actual weights and path, and prefers a complete safetensors snapshot when available', timestamp: new Date().toISOString().slice(0,16).replace('T',' '), status: 'ACTIVE' },
=======
