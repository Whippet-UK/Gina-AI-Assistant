...
import { Activity, Brain, CheckCircle2, Cpu, Database, FileCog, Film, Gauge, HardDrive, Image as ImageIcon, Network, RefreshCw, Server, ShieldCheck, Sparkles, Video, XCircle, Zap, Globe, AlertTriangle } from 'lucide-react';
...
export const LocalCapabilityPanel: React.FC<Props> = ({ onAddLog }) => {
  ...
  const load = async () => {
    setLoading(true); try {
      const r = await fetch('/api/capabilities', { cache: 'no-store' }); const d = await r.json(); if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`); setData(d);
    } catch (e: any) { onAddLog('WARN', `Local capability scan failed: ${e.message}`); } finally { setLoading(false); }
  };
  ...
  return <section className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 shadow-lg">
    ...
    <div className="mb-4 p-2.5 bg-slate-950 border border-slate-800 rounded-lg flex items-center justify-between flex-wrap gap-2">
      <div className="flex items-center gap-2 text-[10px] font-mono">
        <Globe className="w-3.5 h-3.5 text-sky-400" />
        <span className="text-slate-400">Agent Search Router Status:</span>
        <span className="text-sky-400 font-bold">Multi-Engine Failover Stack Active</span>
      </div>
      <div className="flex items-center gap-3 text-[9px] font-mono">
        <span className="text-slate-500">Security Guard: <span className="text-emerald-400 font-bold">Private LAN Protection Enabled</span></span>
        <span className="text-slate-500">Scraping Fallbacks: <span className="text-emerald-400 font-bold">Resilient JSON APIs</span></span>
      </div>
    </div>
            ...
            <div key={g.id} className="p-1.5 rounded bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 transition-colors">
              <div className="flex items-center justify-between gap-1.5">
                <span className="text-slate-200 font-bold truncate flex items-center gap-1.5">
                  <span className={`text-[8px] px-1 py-0.2 rounded border font-mono uppercase ${g.type === 'image' ? 'bg-emerald-950/80 text-emerald-300 border-emerald-800' :
                    g.type === 'video' ? 'bg-indigo-950/80 text-indigo-300 border-indigo-800' :
                    'bg-cyan-950/80 text-cyan-300 border-cyan-800'
                    }`}>{g.type}</span>
                  {g.label}
                </span>
                <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold shrink-0 ${g.status === 'validated' ? 'bg-emerald-950 text-emerald-400 border border-emerald-800' :
