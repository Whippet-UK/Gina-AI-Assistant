...
export const GifStudio: React.FC<GifStudioProps> = ({ telemetry, onAddLog }) => {
  ...
  const [exportUrls, setExportUrls] = useState<{gif?:string; mp4?:string; apng?:string}>({});
  const [previewFormat, setPreviewFormat] = useState<'gif'|'mp4'|'apng'>('gif');
  ...
  const sourceUrl = (previewFormat === 'mp4' ? exportUrls.mp4 : previewFormat === 'apng' ? exportUrls.apng : exportUrls.gif) || activeOutputUrl || activeAsset?.url || '';
  ...
  const exportFormat = async (format:'gif'|'mp4'|'apng') => {
  ...
  const autoFinalize = async (jobId: string) => {
    ...
    try {
      ...
      const results: any = {};
      for (const format of ['gif','mp4','apng'] as const) {
  ...
  return <div className="space-y-4">
    <div className="grid grid-cols-1 xl:grid-cols-[380px_minmax(0,1fr)_390px] gap-4 items-start">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-950/80 p-4 xl:sticky xl:top-4 flex flex-col space-y-3">
        <div className="flex items-center justify-between mb-1"><div><div className="text-[9px] tracking-[.25em] text-cyan-400 font-bold">LIVE PLAYER + TIMELINE HUD</div><div className="text-sm text-slate-200 font-semibold">Frame range {startFrame} → {endFrame}</div></div><div className="flex items-center gap-2 text-[9px] font-mono text-slate-500"><button onClick={()=>setPreviewFormat('gif')} className={`px-2 py-1 rounded border ${previewFormat==='gif'?'border-emerald-500/50 text-emerald-300':'border-slate-800'}`}>GIF {exportUrls.gif?'✓':''}</button><button onClick={()=>setPreviewFormat('mp4')} className={`px-2 py-1 rounded border ${previewFormat==='mp4'?'border-cyan-500/50 text-cyan-300':'border-slate-800'}`}>MP4 {exportUrls.mp4?'✓':''}</button><button onClick={()=>setPreviewFormat('apng')} className={`px-2 py-1 rounded border ${previewFormat==='apng'?'border-emerald-500/50 text-emerald-300':'border-slate-800'}`}>APNG {exportUrls.apng?'✓':''}</button><span>{fps} FPS</span><span>{Math.round(1000/fps)}ms/frame</span></div></div>
